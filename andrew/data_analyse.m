% data_analyse(data, pdf_type, draw, name, foo) - returns [mean, variance, chi2]
...
% Could make improve plotting for 'gauss' and 'poiss', but not needed

%needs	histogram_analyse.m
%				chi_squared.m

function [average, variance, chi2] = data_analyse (data, pdf_type, draw, name, foo)
	...
	variance = sum( col_var .* (0:(cols-1)) )/num_points; %should account for other variances
	
	%Plot data if needed	
	if draw & foo
		fig = figure('visible',visible_plots); %switch vis to off if it becomes a hassle
	...
	if strcmpi(pdf_type, 'both') | strcmpi(pdf_type, 'b')
		...
		if draw
			...
			errorbar(0:(cols-1), col_mean, sqrt(col_var)/2,'.k'); %plot error bars on top
			legend('Sample trial', 'Average values', 'Ideal Gaussian', 'Ideal Poisson',gauss_string,poiss_string)
			%save figure
			saveas(fig, strcat('figures/',strcat(name,'.fig')), 'fig');
			saveas(fig, strcat('figures/eps/',strcat(name,'.eps')), 'epsc');
		end
		
	else	
		%%%%%%%%%%%%%%%%%%%%% Generate pdf data %%%%%%%%%%%%%%%%%%%%
		
		normalization = num_points; % only changes later for gaussian
		%Handle Gaussian
		if strcmpi(pdf_type, 'gauss')
			...
			pdf = normpdf((0:(cols-1)), average, sqrt(variance));
			%normalization = trapz(pdf)*num_points; %wrong idea
			normalization = normalization / trapz(pdf);
		...
		elseif strcmpi(pdf_type, 'poiss')
			...
			%normalization = trapz(pdf)*num_points; %poisson shouldnt need normalization - what was i thinking
			%fprintf('Normalization: %d\n', normalization/num_points);
			pdf = pdf*num_points;
		end
		...
		if draw %Draw the fitted pdf's
			if strcmpi(pdf_type, 'gauss')
				...
				pdf = normpdf((0:step:cols), average, sqrt(variance))*normalization; %smooth the gaussian
				color = 'red';
				name = 'Ideal Gaussian';
